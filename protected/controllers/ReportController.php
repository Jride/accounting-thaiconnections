...
class ReportController extends CController
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', 
				'actions'=>array('report', 'cronreports'),
...
//			$model->phpSecondaryInfo=$_POST['Report']['selectSql'];
		...
		public function actionCronreports()
	{
		...
		$sql = "SELECT Account.id AS accountid, Account.code AS accountcode, Account.name AS accountname, Account.email AS email, Account.balance_threshold AS threshold, Account.days AS days, Account.report_id AS reportID
				FROM Account
				WHERE ( NULLIF(Account.email, '') IS NOT NULL)";
				// WHERE (Account.email BETWEEN '101000' AND '102999')
		...
		foreach($accounts as $account)
		{
			...
			{
				if(!empty($account['days']))
				{
					echo "Level 3";
					$accountName = $account['accountname'];
					$days = $account['days'];
					$dateString = "now -".$days." days";
					//echo "DAYS ---".$dateString;
					//$reportID = $account['reportID'];
					$reportID = '3';
					$start = Date('j/m/Y', strtotime($dateString));
					$end = Date('j/m/Y');
					$accParam = 'AND Account.id='.$account['accountid'];
					$email = $account['email'];
					...
					*/
					//$this->post_to_url("http://accounting.thaiconnections.org/index.php?r=report/report", $data);
					
					//$cmd = "wget --load-cookies cookies2.txt \ --post-data 'reportId=3&0=$accountName&5=$start&6=$end&7=$accParam&printoutview=1&DownloadPDF=Download PDF&cron=yes' \ http://accounting.thaiconnections.org/index.php?r=report/report";
					//$this->generateReport($reportID, $accountName, $start, $end, $accParam, 1, "Download PDF", "yes", $email);
					$_POST['reportId'] = $reportID;
					$_POST['0'] = $accountName;
					$_POST['5'] = $start;
					$_POST['6'] = $end;
					$_POST['7'] = $accParam;
					$_POST['printoutview'] = 1;
					$_POST['cron'] = 'yes';
					$_POST['email'] = $email;
		...
		//			echo $this->_model->selectSql.'<br><br>';
		//			echo $sqlSelect; die();
					...
					// if from cron job
					if(isset($_POST['cron'] ) ){
						$accountName_2 = str_replace(' ', '_', $accountName);
						$pdf_name = "reports/FCF_Report_".$accountName_2."_".date('Y-m-d_H.i.s');
						$cron_job = TRUE;
					}		
					...
					new lazy8tcpdf($this->_model,
						...
						if(isset($_POST['cron'] ) ){
							...
							$message = '<html><body>';
							$message .= '<p>Hello,</p>';
							$message .= '<p>This a automated notification from Thaiconnections Accounting website to inform you that your weekly report for: <strong>'.$accountName.' </strong>is ready to be downloaded. Please click the link to view your report.<p>';
							$message .= '<p>http://accounting.thaiconnections.org/'.$pdf_name.'.pdf<p>';
	...
	private function post_to_url($url, $data) {
       ...
       curl_setopt($post, CURLOPT_COOKIEFILE, "cookies2.txt");
       curl_setopt($post, CURLOPT_COOKIEJAR, "cookies2.txt");
	...
	function generateReport($reportId, $zero, $five, $six, $seven, $printoutview, $DownloadPDF, $cron, $email)
	{
		echo $reportId."<br>";
		echo $zero."<br>";
		echo $five."<br>";
		echo $six."<br>";
		echo $seven."<br>";
		echo $printoutview."<br>";
		echo $DownloadPDF."<br>";
		echo $cron."<br>";
		echo $email."<br>";
		$_POST['DownloadPDF'] = $DownloadPDF;
		$_POST['reportId'] = $reportId;
		$_POST['0'] = $zero;
		$_POST['5'] = $five;
		$_POST['6'] = $six;
		$_POST['7'] = $seven;
		$_POST['printoutview'] = $printoutview;
		$_POST['cron'] = $cron;
		$_POST['email'] = $email;
			...
			if(isset($_POST['DownloadExcel'] ) ){
				...
				new lazy8tcpdf($this->_model,
					...
					if(isset($_POST['cron'] ) ){
						...
						$message = '<html><body>';
						$message .= '<p>Hello,</p>';
						$message .= '<p>This a automated notification from Thaiconnections Accounting website that your weekly report for: <strong>'.$_POST['0'].' </strong>is ready to be downloaded. Please click the link to view your report.<p>';
