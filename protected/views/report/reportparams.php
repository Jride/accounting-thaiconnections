...
if($accountTags != ''){
	...
	if(!in_array('1001', $accountPieces)){
		$accountPieces[] = '1001';
		$accountTags = implode(',', $accountPieces);
	}
	$account_ids = Yii::app()->db->createCommand(
		'SELECT DISTINCT aca.accountId as AccountID FROM AccountCustomerAssignment aca                                                    INNER JOIN (SELECT c.id, c.name FROM Customer c WHERE c.code IN ('.$accountTags.')) ww ON aca.customerId = ww.id ORDER BY AccountID')->queryAll();
		$accountID_string = '';
		$i = 0;
